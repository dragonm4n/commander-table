...
export type Room = {id:string;name:string;status:string;error?:string;mySeat:number;aiOnly?:boolean;seats:{name:string;human:boolean;ready:boolean;connected:boolean;deck:string;commander:string;ai?:AiCoverage}[];game?:Game;chat:{name:string;message:string;at:number}[]};
export type Session = {room:string;seat:number;token:string;invite?:string;resumeKey?:string;server:string};
export type Health = {ready:boolean;engine:string;decks:{id:string;name:string;commander:string;ai?:AiCoverage;kind?:string;base?:string;source?:string;changes?:{from:string;to:string;count:number}[]}[]};
