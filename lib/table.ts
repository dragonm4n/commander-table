export type AiCoverage = {percent:number;total:number;flagged:number;warnings:{name:string;count:number;commander:boolean}[]};
export type Card = {id:number;name:string;hidden?:boolean;imageName?:string;type?:string;text?:string;mana?:string;power?:number;toughness?:number;loyalty?:string;creature?:boolean;land?:boolean;tapped?:boolean;attacking?:boolean;blocking?:boolean;selected?:boolean;selectable?:boolean;actionable?:boolean;sick?:boolean;token?:boolean;commander?:boolean;transformed?:boolean;damage?:number;counters?:Record<string,number>;zone?:string;tokenImages?:string[];controllerSeat?:number;exiledWith?:{id:number};attachedTo?:Target};
export type Zone = {count:number;cards:Card[]};
export type Player = {zoneMotions?:{id:number;zone:string}[];seat:number;id:number;name:string;life:number;lost:boolean;monarch?:boolean;selectedDefender?:boolean;zones:Record<string,Zone>;mana:{color:number;amount:number}[];counters:Record<string,number>;commanders:{name:string;from:number;damage:number;casts:number}[];announcements?:{id:number;stackId?:number;kind:string;card:Card;text:string}[];lastAction?:{id:number;stackId?:number;kind:string;card:Card;text:string};statuses?:{name:string;title:string;text:string}[]};
export type Decision = {id:string;kind:string;presentation?:'library'|'dock'|'ability';source?:Card;message:string;min:number;max:number;options:{id:number;label:string;card?:Card;stackId?:number;seat?:number}[]};
...
export type Game = {confirmation?:boolean;instanceId?:number;controlVersion:number;canAutoPass?:boolean;targeting?:{min:number;max:number;selected:number;source?:Card};promptCard?:Card;cancelEndsTurn?:boolean;winnerSeat?:number;message:string;ok:{label:string;enabled:boolean};cancel:{label:string;enabled:boolean};decision?:Decision;turn:number;round:number;phase:string;activeSeat:number;over:boolean;winner:string;players:Player[];stack:StackItem[];combat?:{kind:'attack'|'block';source:Target;target:Target}[];log:string[];sounds?:{id:number;name:string}[]};
export type AiSeries = {id:string;total:number;completed:number;draws:number;running:boolean;fast:boolean;stopRequested:boolean;stopped:string;players:{seat:number;name:string;deck:string;wins:number;games:number}[];decks:{deck:string;wins:number;games:number}[];results:{game:number;winners:number[];draw:boolean;turns:number;seconds:number}[]};
export type Room = {series?:AiSeries;id:string;name:string;status:string;error?:string;mySeat:number;aiOnly?:boolean;seats:{name:string;human:boolean;ready:boolean;connected:boolean;deck:string;commander:string;ai?:AiCoverage}[];game?:Game;chat:{name:string;message:string;at:number}[]};
export type Session = {room:string;seat:number;token:string;invite?:string;resumeKey?:string;server:string};
export type Health = {ready:boolean;engine:string;decks:{id:string;name:string;commander:string;ai?:AiCoverage;kind?:string;base?:string;source?:string;cards?:{name:string;count:number;type:string;mana:string}[];changes?:{from:string;to:string;count:number}[]}[]};
