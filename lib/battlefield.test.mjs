...
test('exiled cards sit beneath their source without becoming permanents or exposing hidden identity',()=>{
 const host=card(1,{token:false}),p=player(0,[host]);
 const hidden={id:2,name:'Face-down card',hidden:true,zone:'Exile',exiledWith:{id:1}};
 p.zones.Exile={count:2,cards:[hidden,{id:3,name:'Other exile',exiledWith:{id:99}}]};
 const piles=battlefieldPiles([p]).get(0);
 assert.equal(piles.length,1);assert.deepEqual(piles[0].exiled,[hidden]);assert.equal(piles[0].attachments.length,0);
 p.zones.Battlefield.cards=[];assert.equal(battlefieldPiles([p]).size,0);
});
