...
const deck={name:'Meu Muzzio',commander:'Muzzio, Visionary Architect',list:'1 Sol Ring\n98 Island'};
...
test('saved import survives a new storage reader and updating it does not duplicate the list',()=>{
 ...
 const renamed=saveImportedDeck({...deck,name:'Muzzio artefatos'},disk);
 assert.equal(renamed.length,1);
 assert.equal(renamed[0].id,saved[0].id);
 assert.equal(readDecks(disk)[0].name,'Muzzio artefatos');
});
...
test('backup restores the full list on another browser and merges without duplicating decks',()=>{
 ...
 saveImportedDeck({...deck,name:'Outra lista',list:'99 Island'},second);
