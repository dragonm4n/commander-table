# Cartas sinalizadas pelo Forge como problemáticas para a IA

Extração: 2026-09-09. Commander Table alpha 0.5 / gameplay preview. Forge 2.0.15-SNAPSHOT, revisão `53a103721d627ecb76a2ea52b2febe894844f288`.

**Total: 2500 nomes únicos sinalizados**, considerando as bases comum e de variantes da instalação. 33682 nomes examinados nas duas bases. Edições e artes repetidas foram agrupadas pelo nome canônico das regras.

Critério: `CardRules.getAiHints().getRemAIDecks() == true`, a mesma marcação usada por `Deck.getUnplayableAICards()` nos avisos do Forge.

Esta é a lista completa dessa marcação na base instalada, não uma auditoria de todas as decisões da IA. O aviso não significa que a carta nunca seja utilizada. Cartas sem a marcação também podem ser mal jogadas. A marcação não fornece uma explicação individual do problema. A base de variantes pode incluir cartas de formatos especiais; a lista não foi filtrada por legalidade em Commander. Tokens não fazem parte desta extração.

## Base comum — 2481 cartas sinalizadas
...
## Base de variantes — 19 cartas sinalizadas
