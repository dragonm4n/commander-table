...
export default function DecisionPanel({decision:d,busy,onAnswer,onInspect,popup=false,draft,onDraftChange}:{decision:Decision;busy:boolean;onAnswer:(body:Record<string,unknown>)=>void;onInspect:(c:Card)=>void;popup?:boolean;draft?:DecisionDraft;onDraftChange?:Dispatch<SetStateAction<DecisionDraft>>}){
 ...
 return <section className={`decision-panel ${stack?'stack-decision':''} ${popup?'popup-decision':''}`}>
  ...
  <div className="decision-options">{d.options.map((o,i)=>({o,i})).filter(({o})=>[o.label,o.card?.name,o.card?.type].join(' ').toLowerCase().includes(search.toLowerCase())).map(({o,i})=><div className="decision-row" key={o.id}>
   <button className={`decision-option ${selected.includes(o.id)?'chosen':''}`} disabled={busy||allocation} aria-pressed={reveal?undefined:selected.includes(o.id)} onClick={()=>reveal?o.card&&onInspect(o.card):toggle(o.id)}>
