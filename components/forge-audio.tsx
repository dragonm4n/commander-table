...
export default function ForgeAudio({game,roomId}:{game?:Game;roomId?:string}){
 ...
 const toggle=async()=>{if(enabled){setEnabled(false);await context.current?.suspend();return;}try{const ctx=context.current??new AudioContext();context.current=ctx;await ctx.resume();setError('');setEnabled(true);}catch{setError('O navegador não conseguiu ativar o som.');}};
 useEffect(()=>{
  ...
  for(const sound of sounds.filter(s=>s.id>previous).slice(-8)){
   ...
   let buffer=buffers.current.get(sound.name);if(!buffer){buffer=fetch(`/sounds/${sound.name}.mp3`).then(r=>{if(!r.ok)throw new Error('Som indisponível');return r.arrayBuffer()}).then(b=>ctx.decodeAudioData(b));buffers.current.set(sound.name,buffer);}
 ...
 return <button onClick={toggle} aria-pressed={enabled} title={error||'Ativar ou silenciar os efeitos sonoros do Forge'} aria-label={enabled?'Silenciar efeitos do Forge':'Ativar efeitos do Forge'}>{enabled?<Volume2 size={16}/>:<VolumeX size={16}/>}<span>Som</span></button>;
}
