...
public final class WebGui extends BaseWebGui {
 ...
 private volatile String message="Aguardando a partida",okLabel="Confirmar",cancelLabel="Cancelar";
 ...
 public synchronized void refresh(){
  try{
   ...
   if(g!=null){state.put("turn",g.getTurn());state.put("phase",g.getPhase()==null?"":g.getPhase().name());state.put("activeSeat",room.seatFor(g.getPlayerTurn()));state.put("over",g.isGameOver());state.put("winner",g.getWinningPlayerName());
 ...
 Map<String,Object> target(GameEntityView entity){
  ...
  if((c.getZone()==ZoneType.Hand||c.getZone()==ZoneType.Library)&&!c.canBeShownTo(getCurrentPlayer()))return obj("kind","zone","seat",controller,"zone",zone,"name","Carta em zona oculta");
 ...
 Map<String,Object> card(CardView c,boolean explicitlyRevealed){
  ...
  Map<String,Object> r=obj("id",c.getId(),"hidden",hidden,"name",hidden?"Carta com a face para baixo":c.getName(),"tapped",c.isTapped(),"attacking",c.isAttacking(),"blocking",c.isBlocking(),"selected",isHighlighted(c),"selectable",isSelectable(c),"actionable",isWeaklySelectable(c),"zone",c.getZone()==null?"":c.getZone().name(),"counters",counters(c));
 ...
 @Override public void flashIncorrectAction(){message="Essa ação não é válida neste momento. "+message;refresh();}
 ...
 public void action(JsonObject b){
  require(prompt==null,"Responda à escolha pendente.");require(integer(b,"controlVersion",-1)==controlVersion,"A situação mudou; tente novamente.");
  var ctl=getGameController();require(ctl!=null,"Controle do jogador indisponível.");String a=str(b,"action","");
  switch(a){
   case "ok":require(okEnabled,"Confirmação indisponível.");ctl.selectButtonOk();break;
   case "cancel":require(cancelEnabled,"Cancelar indisponível.");ctl.selectButtonCancel();break;
   case "card":CardView c=knownCards.get(integer(b,"cardId",-1));require(c!=null,"Carta não encontrada.");require(c.canBeShownTo(getCurrentPlayer())||c.getZone()==ZoneType.Battlefield,"Carta não disponível.");ctl.selectCard(c,null,CLICK);break;
   case "player":int target=integer(b,"playerId",-1);PlayerView p=null;for(PlayerView x:getGameView().getPlayers())if(x.getId()==target)p=x;require(p!=null,"Jogador não encontrado.");ctl.selectPlayer(p,CLICK);break;
   case "mana":int color=integer(b,"color",-1);require(Set.of(1,2,4,8,16,32).contains(color),"Mana inválida.");ctl.useMana((byte)color);break;
   case "attackAll":ctl.alphaStrike();break;
   case "undo":ctl.undoLastAction();break;
   case "concede":ctl.concede();break;
   default:throw new IllegalArgumentException("Ação não reconhecida.");
  }refresh();
 }
 static final class Decision {
  ...
  Decision(WebGui gui,String kind,String text,List<?> choices,int min,int max,FSerializableFunction display){
   ...
   for(int i=0;i<this.options.size();i++){
    ...
    if(c instanceof CardView cv){Map<String,Object> visible=gui.card(cv,kind.equals("reveal"));o.put("card",visible);if(Boolean.TRUE.equals(visible.get("hidden")))o.put("label","Carta com a face para baixo");}opts.add(o);
   }
   publicData=obj("id",id,"kind",this.kind,"message",text,"min",min,"max",max,"options",opts);
  }
 }
 JsonObject ask(String kind,String msg,List<?> choices,int min,int max,FSerializableFunction display){
  ...
  try{return d.future.get();}catch(Exception e){throw new IllegalStateException("Escolha interrompida.",e);}finally{if(prompt==d)prompt=null;controlVersion++;refresh();}
 }
 public void answer(JsonObject b){Decision d=prompt;require(d!=null&&d.id.equals(str(b,"decisionId","")),"Essa escolha já foi respondida ou mudou.");
  if(d.kind.equals("number")){int v=integer(b,"value",Integer.MIN_VALUE);require(v>=d.min&&v<=d.max,"Número fora do intervalo.");}
  else if(d.kind.equals("text")){require(str(b,"value","").length()<=500,"Resposta muito longa.");}
  else if(d.kind.equals("allocation")){JsonArray values=b.has("values")?b.getAsJsonArray("values"):new JsonArray();require(values.size()==d.options.size(),"Informe todas as quantidades.");int total=0;for(JsonElement e:values){int v=e.getAsInt();require(v>=d.min&&v<=d.max,"Quantidade inválida.");total+=v;}require(total==d.max,"Distribua exatamente "+d.max+".");}
  else {JsonArray ids=b.has("selected")?b.getAsJsonArray("selected"):new JsonArray();require(ids.size()>=d.min&&ids.size()<=d.max,"Escolha entre "+d.min+" e "+d.max+" opções.");Set<Integer> used=new HashSet<>();for(JsonElement e:ids){int id=e.getAsInt();require(id>=0&&id<d.options.size()&&used.add(id),"Opção inválida ou repetida.");}}
  require(d.future.complete(b),"Essa escolha já foi respondida.");
 }
 ...
 @Override public SpellAbilityView getAbilityToPlay(CardView card,List<SpellAbilityView> a,ITriggerEvent e){
  ...
  return choose("select","Escolha a habilidade",choices,1,1,null).get(0);
 }
 ...
 @Override public List<CardView> manipulateCardList(String title,Iterable<CardView> cards,Iterable<CardView> movable,boolean top,boolean bottom,boolean anywhere){List<CardView> all=new ArrayList<>(),move=new ArrayList<>();cards.forEach(all::add);movable.forEach(move::add);List<CardView> ordered=choose("order",title+" — selecione na ordem desejada",move,move.size(),move.size(),null);boolean putTop=top&&(!bottom||showConfirmDialog("Colocar estas cartas no topo?","Ordenar","Topo","Fundo",true));all.removeAll(move);if(putTop)all.addAll(0,ordered);else all.addAll(ordered);return all;}
 @Override public Map<CardView,Integer> assignCombatDamage(CardView attacker,List<CardView> blockers,int damage,GameEntityView defender,boolean override,boolean maySkip){
  ...
  while(true){JsonObject a=ask("allocation","Distribua "+damage+" de dano de "+attacker.getName()+". Atribua dano letal aos bloqueadores antes de atingir o defensor.",options,0,damage,c->c==null?defender.getName():((CardView)c).getName());
   ...
   if(valid)return result;message="Distribuição inválida: atribua dano letal antes de passar ao próximo alvo.";
  }
 }
 @Override public Map<Object,Integer> assignGenericAmount(CardView source,Map<Object,Integer> target,int amount,boolean atLeastOne,String label){
  ...
  while(true){JsonObject a=ask("allocation",label+" — respeite o limite indicado em cada alvo",opts,atLeastOne?1:0,amount,c->String.valueOf(c)+" (máx. "+(target.get(c)==null?amount:target.get(c))+")");Map<Object,Integer> out=new HashMap<>();int i=0;boolean valid=true;
